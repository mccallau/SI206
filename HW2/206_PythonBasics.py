...
# Section Day/Time: Section 3 Thursday 830-930am
# People you worked with: NA
...
def task0():
    print("Austin McCall") #Prints name
    print("Expected Score: 60\n") #Prints expected score
...
def string_manip(s):
    s.strip().upper().replace(" ","#").replace("UMSI","")[::-1] #modifies the input string to the specifications
    return s
...
def name_counts(names):
    ...
    for strings in names:
        if strings in namedict: namedict[strings]+=1 #Creates a dictionary out of the list of strings with keys being the strings and values being the counts
        else: namedict[strings] = 1
    tuplist = sorted(sorted(list(namedict.items()),key = lambda x:x[0]), #creates a list of tuples out of the dictionary, each tuple has a string at index 0 and the count of the string at index 1
    key = lambda x:x[1],reverse = True) #list of tuples is also sorted by the string (a-z) first and then by the count (greatest to least)
    return tuplist
...
def build_acronym(ls):
    x=""
    for st in ls: #iterates through a list of strings and appends the first letter of each string to an empty string in uppercase format
        x += st[0].upper() 
    return x
...
class House(object):
    ...
    def determine_size(self): #takes the color of the house and determines the house's size
